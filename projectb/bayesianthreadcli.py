"""
The bayesian optimization module for the Command Line Interface. Parts of the module
...
# The Bayesian optimization class, requires parameters and a console
# to write to.
class BayesianOptProcess():
    ...
    def run(self):
        ...
        for i, e in enumerate(self.experiments):
            ...
            try:
                ...
                self.console("Finished with " + e["policy"][0] + " and " + e["kernel"] + " kernel.", 1)
            except:
                self.console("An error occured during Bayesian Optimization Experiment, please check your settings")
        self.console("Ended Bayesian Optimization",1)
    ...
    def initialmodel(self, objective, init, bounds, kernel):
        ...
        try:
            ...
            self.console("The initial number of samples must be a number", 0)
        if n > 0:
            self.console("Starting initializer", 2)
            X = init(bounds, n=n)
            Y = [objective(x) for x in X]
            self.console("Finished initializer", 2)
        elif data == [[], []]:
