from Tkinter import Frame, Toplevel, Message,CENTER,LEFT,BOTH,RIGHT,Label,W,E,S,N,Tk,NW,StringVar,OptionMenu,DISABLED,NORMAL,FLAT
from factories import customvar,camobutton,qbutton,header,headersmall,infoh,infov
...
class observation():
    ...
    def experimentdata(self):
        ...
        Label(resultsframe,text="Current:", font=Styles.fonts["entry"],bg=Styles.colours["grey"],width=9).grid(row=0,column=1,sticky=E)
        ...
        Label(resultsframe,text="Overall:", font=Styles.fonts["entry"],bg=Styles.colours["grey"],width=9).grid(row=1,column=1,sticky=E)
        self.totbestlabel= Label(resultsframe,text = self.iniparams["totbest"],bg=Styles.colours["grey"],width=20, font=Styles.fonts["h2"])
        ...
        Label(curresultsframe,text="y:", font=Styles.fonts["entry"],bg=Styles.colours["grey"],width=9).grid(row=0,column=1,sticky=E)
        ...
        Label(curresultsframe,text="mu:", font=Styles.fonts["entry"],bg=Styles.colours["grey"],width=9).grid(row=1,column=1,sticky=E)
        self.curmu= Label(curresultsframe,text = self.iniparams["totbest"],bg=Styles.colours["grey"],width=20, font=Styles.fonts["h2"])
        self.curmu.grid(row=1,column=2,sticky=W)

        Label(curresultsframe,text="var:", font=Styles.fonts["entry"],bg=Styles.colours["grey"],width=9).grid(row=2,column=1,sticky=E)
        self.curvar= Label(curresultsframe,text=self.iniparams["totbest"],bg=Styles.colours["grey"],width=20, font=Styles.fonts["h2"])
    ...
    def startgraph(self):
        ...
        self.option = OptionMenu(root, self.graphtype, "Graph: Objective Value", "Graph: Running Best (Zoom)",
                                 ...
                                 "Graph: Objective Value (Last 25)","Graph: Running Best (Last 25)","Graph: Time (seconds)")
        ...
        canvas.show()
        canvas.get_tk_widget().configure(background=Styles.colours["grey"],  highlightcolor=Styles.colours["grey"], highlightbackground=Styles.colours["grey"])
    ...
    def updategraph(self):
        if not self.lockvar:
            ...
            def bestrungraphz():
                self.graphframe.plot([yi if yi > np.max(y[:i+1]) else np.max(y[:i+1]) for i,yi in enumerate(y)], 'b', linewidth=3)        # plot performance
                self.graphframe.axis((0, ylen, ymax*1.5, ymax*1.5 if ymax>0 else ymax*0.5))
                addlabels()

            def bestrungrapho(n):
                bestrun = [yi if yi > np.max(self.iniparams["y"][:i+1]) else np.max(y[:i+1]) for i,yi in enumerate(self.iniparams["y"])]
            ...
            def vargraph(n):
                self.graphframe.plot(self.iniparams["var"][-n:] if len(self.iniparams["var"]) >= n else self.iniparams["var"], 'm', linewidth=3)        # plot performance
                self.graphframe.axis("auto")
                addlabels()
